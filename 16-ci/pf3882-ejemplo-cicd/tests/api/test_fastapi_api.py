...
def tl(fastapi_client):
    ...
    return fastapi_client.post("/api/v1/tasklists", json={"name": "Test List"}).json()


@pytest.fixture
def task(fastapi_client, tl):
    """Tarea creada a través de la API FastAPI."""
    return fastapi_client.post("/api/v1/tasks", json={
...
class TestFastAPITaskListEndpoints:
    def test_list_empty(self, fastapi_client):
        resp = fastapi_client.get("/api/v1/tasklists")
        assert resp.status_code == 200
        assert resp.json() == []

    def test_create_returns_201(self, fastapi_client):
        resp = fastapi_client.post("/api/v1/tasklists", json={"name": "Work"})
        assert resp.status_code == 201

    def test_create_response_shape(self, fastapi_client):
        body = fastapi_client.post(
            "/api/v1/tasklists", json={"name": "Work"}).json()
    ...
    def test_create_missing_name_returns_422(self, fastapi_client):
        ...
        resp = fastapi_client.post("/api/v1/tasklists", json={})
        assert resp.status_code == 422

    def test_list_after_create(self, fastapi_client, tl):
        body = fastapi_client.get("/api/v1/tasklists").json()
        assert len(body) == 1
        assert body[0]["id"] == tl["id"]

    def test_get_by_id(self, fastapi_client, tl):
        resp = fastapi_client.get(f"/api/v1/tasklists/{tl['id']}")
        assert resp.status_code == 200
        body = resp.json()
    ...
    def test_get_by_id_includes_tasks(self, fastapi_client, tl, task):
        body = fastapi_client.get(f"/api/v1/tasklists/{tl['id']}").json()
        assert len(body["tasks"]) == 1
        assert body["tasks"][0]["id"] == task["id"]

    def test_get_by_id_not_found(self, fastapi_client):
        assert fastapi_client.get("/api/v1/tasklists/999").status_code == 404

    def test_delete_returns_204(self, fastapi_client, tl):
        resp = fastapi_client.delete(f"/api/v1/tasklists/{tl['id']}")
        assert resp.status_code == 204

    def test_delete_removes_from_list(self, fastapi_client, tl):
        fastapi_client.delete(f"/api/v1/tasklists/{tl['id']}")
        assert fastapi_client.get("/api/v1/tasklists").json() == []

    def test_delete_cascades_tasks(self, fastapi_client, tl, task):
        fastapi_client.delete(f"/api/v1/tasklists/{tl['id']}")
        assert fastapi_client.get("/api/v1/tasks").json() == []

    def test_delete_not_found(self, fastapi_client):
        assert fastapi_client.delete(
            "/api/v1/tasklists/999").status_code == 404
...
class TestFastAPITaskEndpoints:
    def test_list_empty(self, fastapi_client):
        resp = fastapi_client.get("/api/v1/tasks")
        assert resp.status_code == 200
        assert resp.json() == []

    def test_create_returns_201(self, fastapi_client, tl):
        resp = fastapi_client.post("/api/v1/tasks", json={
    ...
    def test_create_response_shape(self, fastapi_client, tl):
        body = fastapi_client.post("/api/v1/tasks", json={
            "description": "Buy milk",
            "tasklist_id": tl["id"],
        }).json()
    ...
    def test_create_missing_tasklist_id_returns_422(self, fastapi_client):
        resp = fastapi_client.post(
            "/api/v1/tasks", json={"description": "Orphan"})
        assert resp.status_code == 422

    def test_list_after_create(self, fastapi_client, task):
        body = fastapi_client.get("/api/v1/tasks").json()
        assert len(body) == 1
        assert body[0]["id"] == task["id"]

    def test_list_filtered_by_tasklist(self, fastapi_client, tl, task):
        tl2 = fastapi_client.post(
            "/api/v1/tasklists", json={"name": "Other"}).json()
        fastapi_client.post(
            "/api/v1/tasks", json={"description": "Other", "tasklist_id": tl2["id"]})
        result = fastapi_client.get(
            f"/api/v1/tasks?tasklist_id={tl['id']}").json()
        assert len(result) == 1
        assert result[0]["tasklist_id"] == tl["id"]

    def test_get_by_id(self, fastapi_client, task):
        resp = fastapi_client.get(f"/api/v1/tasks/{task['id']}")
        assert resp.status_code == 200
        assert resp.json()["id"] == task["id"]

    def test_get_by_id_not_found(self, fastapi_client):
        assert fastapi_client.get("/api/v1/tasks/999").status_code == 404

    def test_update_description(self, fastapi_client, task):
        resp = fastapi_client.put(
            f"/api/v1/tasks/{task['id']}", json={"description": "Updated"})
        assert resp.status_code == 200
    ...
    def test_update_all_statuses(self, fastapi_client, task):
        for status in ("pending", "in_progress", "done", "cancelled"):
            body = fastapi_client.put(
                f"/api/v1/tasks/{task['id']}", json={"status": status}).json()
    ...
    def test_update_not_found_returns_404(self, fastapi_client):
        resp = fastapi_client.put(
            "/api/v1/tasks/999", json={"description": "x"})
        assert resp.status_code == 404

    def test_update_persists(self, fastapi_client, task):
        fastapi_client.put(
            f"/api/v1/tasks/{task['id']}", json={"description": "Persisted"})
        body = fastapi_client.get(f"/api/v1/tasks/{task['id']}").json()
        assert body["description"] == "Persisted"

    def test_delete_returns_204(self, fastapi_client, task):
        resp = fastapi_client.delete(f"/api/v1/tasks/{task['id']}")
        assert resp.status_code == 204

    def test_delete_removes_task(self, fastapi_client, task):
        fastapi_client.delete(f"/api/v1/tasks/{task['id']}")
        assert fastapi_client.get(
            f"/api/v1/tasks/{task['id']}").status_code == 404

    def test_delete_not_found(self, fastapi_client):
        assert fastapi_client.delete("/api/v1/tasks/999").status_code == 404
