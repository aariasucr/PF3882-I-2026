...
Utiliza FastAPI TestClient para hacer POST a /graphql (Strawberry sobre FastAPI).
...
class TestGraphQLQueries:
    ...
    def test_tasklists_includes_tasks(self, graphql_client, tl, task):
        data, errors = gql(
            graphql_client, "{ tasklists { id tasks { id description } } }")
        assert errors is None
    ...
    def test_tasks_returns_created(self, graphql_client, task):
        data, errors = gql(
            graphql_client, "{ tasks { id description status tasklistId } }")
        assert errors is None
    ...
    def test_tasks_filtered_by_tasklist(self, graphql_client, tl, task):
        ...
        data2, _ = gql(
            graphql_client, 'mutation { createTasklist(name: "Other") { id } }')
...
class TestGraphQLMutations:
    ...
    def test_delete_tasklist_removes_record(self, graphql_client, tl):
        gql(graphql_client, "mutation D($id: Int!) { deleteTasklist(id: $id) }", {
            "id": tl["id"]})
    ...
    def test_delete_tasklist_not_found(self, graphql_client):
        data, errors = gql(
            graphql_client, "mutation { deleteTasklist(id: 999) }")
        assert errors is None
    ...
    def test_delete_task_removes_record(self, graphql_client, task):
        gql(graphql_client, "mutation D($id: Int!) { deleteTask(id: $id) }", {
            "id": task["id"]})
        data, _ = gql(graphql_client, "query G($id: Int!) { task(id: $id) { id } }", {
                      "id": task["id"]})
        assert data["task"] is None
