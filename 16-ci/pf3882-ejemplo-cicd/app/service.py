...
class TaskListService:
    ...
    def restore(self, id: int) -> bool:
        with self._sf() as session:
            return TaskListRepository(session).restore(id)

    def purge(self, id: int) -> bool:
        with self._sf() as session:
            return TaskListRepository(session).purge(id)
...
class TaskService:
    ...
    def restore(self, id: int) -> bool:
        with self._sf() as session:
            return TaskRepository(session).restore(id)

    def purge(self, id: int) -> bool:
        with self._sf() as session:
            return TaskRepository(session).purge(id)
